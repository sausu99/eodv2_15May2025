...
function adminUser($username, $password){
    ...
    $sql = "SELECT id,username FROM tbl_vendor where username = '".$username."' and password = '".password_hash($password, PASSWORD_BCRYPT)."'";       
    $result = mysqli_query($mysqli,$sql);
    $num_rows = mysqli_num_rows($result);
     
    if ($num_rows > 0){
        while ($row = mysqli_fetch_array($result)){
            echo $_SESSION['seller_id'] = $row['id'];
                        echo $_SESSION['seller_username'] = $row['username'];
